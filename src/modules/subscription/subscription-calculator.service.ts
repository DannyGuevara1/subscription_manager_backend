...
export default class SubscriptionCalculatorService
	implements SubscriptionDateCalculator
{
	...
	nextPaymentDate(config: CalculatorConfig): Date {
		try {
			const { firstPaymentDate, billingFrequency, billingUnit, trialEndsOn } =
				config;
			...
			while (
				...
				nextDate = temporalFirstPaymentDate.add({
					[durationStr]: periods * billingFrequency,
				});
				periods++;
			}
	...
	projectNextPaymentDates(config: ProjectionConfig): Date[] {
		...
		let periods = 1;
		while (
			Temporal.ZonedDateTime.compare(temporalCurrent, temporalEndDate) <= 0
		) {
			nextsPaymentDate.push(this.toJsDate(temporalCurrent));
			temporalCurrent = temporalFirstPaymentDate.add({
				[durationStr]: periods * billingFrequency,
			});
			periods++;
		}
